...
import { REQUEST_HEADER_AUTH, REQUEST_UID, TCB_TOKEN_SECRET } from 'utils/config'
...
export const adminGuard = async (ctx: Context, next: Next) => {
  const token = ctx.get(REQUEST_HEADER_AUTH)

  if (!token) respond.fail(401, '您尚未登录，请先登录')

  const tokenInfo = jwt.verify(token, TCB_TOKEN_SECRET)
  if (!tokenInfo[REQUEST_UID]) respond.fail(401, '无法获取管理员信息，请重新登录')
